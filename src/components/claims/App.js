...
import ChangePassword from 'scenes/Login/components/ChangePassword'
...
const AuthComponent = props => {
  ...
  if (!isEmpty(state) &&
    (state.from.pathname.startsWith('/consultar-siniestro/RG') ||
      ...
      state.from.pathname.startsWith('/siniestro/documentos/' ||
      state.from.pathname === '/tareas/siniestro-duplicado/RG19105179/53401'))) {
    if (
      userClaims &&
...
const Routes = ({ childProps }) => (
  <Switch>
    ...
    <ProtectedRoute
      path="/siniestro/documentos/:idDocumento"
      exact
      render={DescargaDocumento}
      props={childProps}
    />
...
class App extends Component {
  ...
  onStorageEvent = storageObject => {
    ...
    if (storageObject.key.endsWith('LastAuthUser')) {
      ...
      if (
        userCognito &&
        ![userCognito.attributes.email, userCognito.attributes.sub].includes(
          lastAuthUser
        )
      ) {
        Modal.warning({
          ...
          content: (
            <span>
              Se cerró la sesión de su cuenta. Presione “Volver a cargar” para
